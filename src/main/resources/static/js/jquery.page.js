...
(function ($) {
  ...
  $.fn.Page = function (options) {
    ...
    return this.each(function () {
      ...
      var url=opts.url;
      var str = '';
      var str1 = '<li><a href="/'+url+'?currPage=1" class="'+ active +'">1</a></li>';
      if (l > 1&&l < n+1) {
        for (i = 2; i < l + 1; i++) {
          str += '<li><a href="/'+url+'?currPage='+i+'">' + i + '</a></li>';
        }
      }else if(l > n){
        for (i = 2; i < n + 1; i++) {
          str += '<li><a href="/'+url+'?currPage='+i+'">' + i + '</a></li>';
      ...
      if(opts.hasLastPage){
          dataHtml += '<div class="last fr" onclick=window.location="/'+url+'?currPage='+l+'">' + opts.lastPage + '</div>';
      }
          dataHtml += '<ul class="pagingUl">' + str1 + str + '</ul>';
      if(opts.hasFirstPage){
          dataHtml += '<div class="first fr" onclick=window.location="/'+url+'?currPage=1">' + opts.firstPage + '</div>';
      }
      if(opts.hasPrv){
      ...
      if(activePage>1){
	      var $this = obj.find('ul li').eq(activePage).find('a');
	      var pageshow = activePage;
	      var nums = odevity(n);
	      opts.callBack(pageshow);
	      if(l>n){
	        if(pageshow > l-nums/2&&pageshow < l+1){
	        ...
	        $('.' + active).removeClass(active);
	        obj.find('ul li').eq(pageshow).find('a').addClass(active);
	      }
      }
        ...
        function fpageShow(activePage){
          ...
          var str1 = '';
          for(i=0;i<n;i++){
            str1 += '<li><a href="/'+url+'?currPage='+(pageStart+i)+'" class="">' + (pageStart+i) + '</a></li>'
          }
        ...
        function fpagePrv(index){
          ...
          if(l>n-1){
            for(i=0;i<n;i++){
              str1 += '<li><a href="/'+url+'?currPage='+(i+1)+'" class="">' + (i+1) + '</a></li>'
            }
          }else{
            for(i=0;i<l;i++){
              str1 += '<li><a href="/'+url+'?currPage='+(i+1)+'" class="">' + (i+1) + '</a></li>'
        ...
        function fpageNext(index){
          ...
          if(l>n-1){
            for(i=l-(n-1);i<l+1;i++){
              str1 += '<li><a href="/'+url+'?currPage='+i+'" class="">' + i + '</a></li>'
            }
           obj.find('ul').html(str1);
           obj.find('ul li').eq(index).find('a').addClass(active);
           //obj.find('ul li').eq(index).find('a').onclick();
          }else{
            for(i=0;i<l;i++){
              str1 += '<li><a href="/'+url+'?currPage='+(i+1)+'" class="">' + (i+1) + '</a></li>'
            }
